	...
	switch(peer->state)
	{
		...
		case GPI_PEER_WAITING:
		{
			...
			if(str != NULL)
			{
				...
				// Was it rejected?
				///////////////////
				char *buffer = peer->inputBuffer.buffer;
				if(strncmp(buffer, "\\anack\\", 7) == 0)
				{
					// Rejected.
					////////////
					peer->nackCount++;
				...
				else if(strncmp(buffer, "\\aack\\", 6) != 0)
				{
					// Unknown message.
					///////////////////
...
)
{
	...
	assertWithLine(peer->state == GPI_PEER_WAITING, 193);
...
GPIPeer * gpiGetConnectedPeer(const GPConnection * connection,
							  int profileid)
{
	GPIConnection * iconnection = (GPIConnection*)*connection;
	GPIPeer * pcurr;
	...
	for(pcurr = iconnection->peerList ; pcurr != NULL ; pcurr  = pcurr->pnext)
	{
		// Check for a match.
		/////////////////////
		if(pcurr->profile == profileid && pcurr->state == GPI_PEER_CONNECTED)
		{
			// Got it.
			//////////
			return pcurr;
...
)
{
	...
	assertWithLine(peer != NULL, 832);
	assertWithLine(message != NULL, 833);
