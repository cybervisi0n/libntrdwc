...
)
{
	...
	assertWithLine(connection, 91);
	assertWithLine(ghiRequestToConnection(connection->request) == connection, 92);
	...
	} else
	{
		...
		// Log buffer data
		ghiLog(connection->getFileBuffer.data, connection->getFileBuffer.len);
...
)
{
	...
	assertWithLine(URL && URL[0], 296);
	assertWithLine(bufferSize >= 0, 297);
	assertWithLine(!buffer || bufferSize, 298);
