 ...
 * * MACROS **
 ***********/
#define ASSERT_SOCK(sock, line)   {\
assertWithLine((sock) != NULL, line);\
assertWithLine(((sock)->connectState == ciNotConnected) ||\
((sock)->connectState == ciConnected) ||\
((sock)->connectState == ciDisconnected), line);\
ASSERT_BUFFER(&(sock)->inputQueue, line);\
ASSERT_BUFFER(&(sock)->outputQueue, line);\
}

#define ASSERT_CONNECTED(sock, line)  assertWithLine((sock)->connectState == ciConnected, line)

#define ASSERT_BUFFER(buffer, line)   {\
assertWithLine((buffer) != NULL, line);\
assertWithLine((buffer)->size >= 0, line);\
assertWithLine(((buffer)->size % BUFFER_INC) == 0, line);\
assertWithLine((buffer)->length >= 0, line);\
assertWithLine((buffer)->length <= (buffer)->size, line);\
}
...
static CHATBool ciBufferPreAppend(ciBuffer * buffer, int len)
{
	...
	ASSERT_BUFFER(buffer, 120);
	assertWithLine(len >= 0, 121);
	assertWithLine(len <= SHRT_MAX, 122); // sanity check
...
{
	ASSERT_BUFFER(buffer, 151);
	assertWithLine(len >= 0, 152);
	assertWithLine(len <= buffer->length, 153);
						 ...
						 int port)
{
	...
	ASSERT_SOCK(sock, 216);
	assertWithLine(serverAddress != NULL, 217);
	assertWithLine(port >= 0, 218);
	assertWithLine(port <= USHRT_MAX, 219);
	assertWithLine(sock->connectState == ciNotConnected, 220);
...
static void ciSocketThinkSend(ciSocket * sock)
{
	...
	ASSERT_SOCK(sock, 340);
	ASSERT_CONNECTED(sock, 341);
...
static void ciSocketThinkRecv(ciSocket * sock)
{
	...
	ASSERT_SOCK(sock, 378);
	ASSERT_CONNECTED(sock, 379);
...
{
	ASSERT_SOCK(sock, 446);
	ASSERT_CONNECTED(sock, 447);
					  ...
					  const char * buffer)
{
	...
	ASSERT_SOCK(sock, 472);
	ASSERT_CONNECTED(sock, 473);
	assertWithLine(buffer != NULL, 474);
					   ...
					   ...)
{
	...
	ASSERT_SOCK(sock, 531);
	ASSERT_CONNECTED(sock, 532);
	assertWithLine(buffer != NULL, 533);
...
CHATBool ciParseParam(const char *pText, ciServerMessage * message)
{
	...
	assertWithLine(pText != NULL, 673);
	assertWithLine(message != NULL, 674);
